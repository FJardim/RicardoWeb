...
import ModalAddRecipeToOverview from "./Overview/ModalAddRecipeToOverview";
...
const ProductInfo = ({
  ...
  const [{ data: createOrderData, loading: createOrderLoading }, createOrder] =
    useAxios(
  ...
  const [
    { paymentMethods, total, numberOfPages, size, error, loading },
    getPaymentMethods,
  ] = usePaymentMethods();
  ...
  const handleFavoriteClicked = (reaction) => () =>
    onFavoriteClicked?.({ type, reaction });
  ...
  return (
    <div className="w-full md:w-1/2 md:px-8">
      <div className="md:flex items-center text-3xl md:justify-between">
        <h1 className="font-bold text-2xl md:ml-1 md:block w-full text-center md:text-left">
          {name}
        </h1>
      ...
      <div className="flex items-center">
        <RatingComponent disabled value={rating} />
        <p
          className="text-gray-300 text-lg m-2 underline cursor-pointer"
          onClick={() => {
            setShowCustomersReviews(true);
          }}
        >
          ...
          {description ? (
            ...
            <>
              ...
              {details?.slice(0, maxDetailsCount).map((detail, i) => (
                <a key={i} style={{ display: "block" }} href={detail.uri}>
                  {detail?.name}
                </a>
              ))}
              {ingredients
                ?.slice(0, maxIngredientsCount)
                .map((ingredient, i) => (
                  <div key={i}>
                    {ingredient.value}{" "}
                    {ingredient.measurementUnit.name.toLowerCase()} of{" "}
        ...
        <div className="text-main text-3xl font-semibold">
          {alreadyAcquired ? (
            <div className="text-center mt-8">
              ...
              <div className="flex items-center justify-center">
                <Link
                  ...
                  className="bg-main text-white px-8 py-2 rounded-xl"
                >
                  Go to my Inventory
                </Link>
              </div>
            ...
            <div className="mt-8 flex items-center justify-between">
              ...
              {isPremiun ? (
                <div className="w-1/2">
                  <h1 className="text-xl text-gray-500 mb-4">Pay with:</h1>
                  <div className="grid grid-cols-1 md:grid-cols-2 md:gap-2 justify-center">
                    {paymentMethods?.map((payment, i) => {
                      return (
                        <button
                          key={i}
                          className="py-2 rounded-xl text-center text-white capitalize"
                          onClick={() => handleBuy(payment?.code)}
                        >
                          <img
                            src={imgUrl(payment?.image)}
                            alt=""
                            style={{ maxWidth: "100%" }}
                          />
                        </button>
                ...
                <button
                  className="bg-main px-8 py-2 rounded-xl text-white"
                  onClick={() => {
                    if (!createOrderData) {
                      handleBuy(null);
                    } else {
              ...
              {haveDiscount && <p className="text-gray-400 text-sm">$48.56</p>}
            </div>
          )}
        </div>
      </div>
      <Modal
        show={showCustomersReviews}
        onClose={() => setShowCustomersReviews(false)}
      >
        <div
          style={{ maxHeight: "70vh", overflowY: "auto" }}
          className="custom-scrollbar custom-scrollbar-main"
        >
              ...
              return (
                <div key={i} className="py-4 border-b border-main">
                  <div className="flex items-center space-x-4">
                    <img
                      src={imgUrl(review?.client?.imgPath)}
          ...
          {loadingRatings ? (
            <div className="text-center my-4">Loading more reviews...</div>
          ) : ratingPages > filters?.page ? (
            <div className="text-center">
              <button
                onClick={handleMore}
                className="bg-white px-8 py-2 rounded-full shadow mt-4 mb-4"
              >
                Load More
              </button>
            </div>
          ) : (
            <div className="text-center my-4">No more reviews.</div>
          )}
        </div>
      </Modal>
    </div>
